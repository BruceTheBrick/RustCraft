...
impl BaseRenderer {
    pub async fn new(window: Arc<Window>) -> Result<Self, RendererError>  {
        ...
        let surface = instance.create_surface(window.clone()).map_err(RendererError::SurfaceCreation)?;
        ...
        ).await.map_err(|_| RendererError::AdapterNotFound)?;
        ...
        ).await.map_err(RendererError::DeviceCreation)?;
